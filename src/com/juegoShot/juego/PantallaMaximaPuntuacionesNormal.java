...
public class PantallaMaximaPuntuacionesNormal extends Pantalla {
	Integer scores[]= new Integer[5];
	//String puestos[]= new String[5];
	//Integer puestos[]= new Integer[5];
	String nicks[]= new String[5];

	public PantallaMaximaPuntuacionesNormal(Juego juego) {
		super(juego);
		for (int i = 0; i < 5; i++){
			//puestos[i]= ConfiguracionesNormal.puestos[i];
			//puestos[i]= ConfiguracionesNormal.puestos[i];
			scores[i]= ConfiguracionesNormal.highscores[i];
			nicks[i]= ConfiguracionesNormal.nicksscores[i];
		}
		...
		for(int i= 0; i < len ; i ++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				if(event.x < 46 && event.y > 434){
					if(ConfiguracionesNormal.soundEnabled)
	...
	//Hay que investigar segun que idioma usemos, utilizar un tipo de letra o otro.
