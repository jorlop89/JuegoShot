...
//Cambiar como pantallamaximapuntuacionfacil
public class PantallaMaximaPuntuacionesDificil extends Pantalla {
	Integer scores[]= new Integer[5];
	//String puestos[]= new String[5];
	//Integer puestos[]= new Integer[5];
	String nicks[]= new String[5];

	public PantallaMaximaPuntuacionesDificil(Juego juego) {
		super(juego);
		for (int i = 0; i < 5; i++){
			//puestos[i] = ConfiguracionesDificil.puestos[i];
			scores[i]= ConfiguracionesDificil.highscores[i];
			nicks[i]= ConfiguracionesDificil.nicksscores[i];
		}
		...
		for(int i= 0; i < len ; i ++){
			...
			if(event.type == TouchEvent.TOUCH_UP){
				if(event.x < 46 && event.y > 434){
					if(ConfiguracionesDificil.soundEnabled)
	...
	public void present(float deltaTime) {
		...
		g.drawPixmap(Assets.botonniveles,60,75,0,111,201,36);
