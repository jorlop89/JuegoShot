...
// Todo lo cambiado en PantallaJuegoFacil si funciona cambiarlo a las otras dos clases.

public class PantallaJuegoFacil extends Pantalla {
		...
		public PantallaJuegoFacil(Juego juego) {
			super(juego);
		...
		private void updateRunning(List<TouchEvent> touchEvents, float deltaTime) {
			...
			for(int i=0; i< len; i++){
				...
				if(event.type == TouchEvent.TOUCH_DOWN) {
					...
					if(inBounds(event,96,85,129,129)){
						if(ConfiguracionesFacil.soundEnabled || ConfiguracionesNormal.soundEnabled || ConfiguracionesDificil.soundEnabled)
							Assets.pulsar.play(1);
			...
			if(mundo.nivelCompletado){
				if(ConfiguracionesFacil.soundEnabled || ConfiguracionesNormal.soundEnabled || ConfiguracionesDificil.soundEnabled)
					...
					//Nuevo
					if(antiguaPuntuacion >= ConfiguracionesFacil.highscores[4]){
						puntuacion = antiguaPuntuacion;
					}else{
						puntuacion = antiguaPuntuacion;
				}
					
				ConfiguracionesFacil.addScore(antiguaPuntuacion,nombre);
			...
			for(int i=0; i < len; i++){
				...
				if(event.type == TouchEvent.TOUCH_DOWN){
					if(event.x >= 232 && event.x < 278 && event.y >= 350 && event.y < 396){  /*Boton Partida Nueva */
						...
						//Nota: Pensar en poner un boton para jugar de nuevo a otra partida, sin salir del juego. Este código funciona: juego.setScreen(new PantallaJuegoNuevo(juego));
		...
		private void drawWorld(MundoJuego mundo) {
			...
			for(int i = 0 ; i < numBolas ; i++){
				...
				/*else if(bola.estadoB == Bola.BOLA_OBJETIVO){
					g.drawPixmap(bolaPixmap, x+96, y+85, 66, 0, 33, 33);
				}*/
		...
		/*Comentario: Todo lo que retoque en pantallaJuegoFacil hay que retocarlo en pantallaJuegoNormal y pantallaJuegoDificil*/
		...
		private void drawRunningUI() {
		    ...
		    g.drawPixmap(Assets.niveljuego, 20, 432, 0, 0, 67, 28);
		...
		private void drawGameLevelCompletedUI() {
			...
			dibujarNumerosAPartirDeInteger(g, puntuacion,165,200);
			g.drawPixmap(Assets.nombre, 50, 150);
			g.drawPixmap(Assets.anadirnombre, 140, 150);   //Cambiar arriba.
		...
		public void pause() {
			...
			if(mundo.nivelCompletado){
				ConfiguracionesFacil.addScore(mundo.puntuacion, nombre);
				ConfiguracionesFacil.save(juego.getFileIO());
			}
