...
public class PantallaJuegoDificil extends Pantalla {
		...
		public PantallaJuegoDificil(Juego juego) {
			super(juego);
		...
		private void updateRunning(List<TouchEvent> touchEvents, float deltaTime) {
			...
			for(int i=0; i< len; i++){
				...
				if(event.type == TouchEvent.TOUCH_DOWN) {
					...
					if(inBounds(event,32,21,257,257)){
						if(ConfiguracionesFacil.soundEnabled || ConfiguracionesNormal.soundEnabled || ConfiguracionesDificil.soundEnabled)
							Assets.pulsar.play(1);
							// Hay que usar el event para marcar la bola, para ello hay que utilizar las coordenadas.
							if(isCollition(event, 32, 21, 257, 257)){
								// Hay que usar el event para marcar la bola, para ello hay que utilizar las coordenadas.
								int columna = ((event.x - 32) / 33);
								int fila = ((event.y - 21) / 33);
				...
				mundo.update(deltaTime);
					...
					if(mundo.nivelCompletado){
						if(ConfiguracionesFacil.soundEnabled || ConfiguracionesNormal.soundEnabled || ConfiguracionesDificil.soundEnabled)
							...
							//Nuevo
							if(antiguaPuntuacion >= ConfiguracionesDificil.highscores[4]){
								puntuacion = antiguaPuntuacion;
							}else{
								puntuacion = antiguaPuntuacion;
							}
							ConfiguracionesDificil.addScore(antiguaPuntuacion,nombre);
		...
		private void drawWorld(MundoJuego mundo) {
			...
			for(int i = 0 ; i < numBolas ; i++){
				...
				x= bola.columna * 32;
				y= bola.fila * 32;
				g.drawPixmap(bolaPixmap, x+32, y+20, 0, 0, 33, 33);
				

				if(bola.estadoB == Bola.BOLA_ELEGIDA){
					g.drawPixmap(bolaPixmap, x+32, y+20, 33, 0, 33, 33);
				...
				/*else if(bola.estadoB == Bola.BOLA_OBJETIVO){
					g.drawPixmap(bolaPixmap, x+32, y+20, 66, 0, 33, 33);
				}*/
		...
		private void drawRunningUI() {
	        ...
	        g.drawPixmap(Assets.niveljuego, 20, 432, 0, 56, 67, 28);
		...
		public void pause() {
			...
			if(mundo.nivelCompletado){
				ConfiguracionesDificil.addScore(mundo.puntuacion, nombre);
				ConfiguracionesDificil.save(juego.getFileIO());
			}
